...
export class WorkflowExecutor {
   ...
   */
  private validateAndResolvePath(stepPath: string): {
    resolvedPath: string;
    isDirectory: boolean;
  } {
  ...
  private async executeOnce(
    ...
    for (let i = 0; i < workflow.steps.length; i++) {
      ...
      try {
        ...
        if (isDirectory) {
          log.step(`Executing directory:`, resolvedPath);
          const dirResult = await this.executeDirectory(resolvedPath, {
            ...options,
            variables,
          });
          ...
          if (dirResult.results.length > 0) {
            const lastResult =
              dirResult.results[dirResult.results.length - 1];
        ...
        const renderedPrompt = TemplateEngine.render(
          promptFile.content,
          variables,
        );
        ...
        const result = await this.backend.execute({
          ...
          streamCallback,
          stepName: path.basename(resolvedPath),
        });
  ...
  private async executeLoop(
    ...
    for (let i = 0; i < loopConfig.count; i++) {
      iterations++;
      log.step(`Loop iteration ${iterations}/${loopConfig.count}`);
      ...
      if (loopConfig.exitOn && result.results.length > 0) {
        ...
        if (this.checkExitCondition(lastResult, loopConfig.exitOn)) {
          log.step(`Exit condition met: ${loopConfig.exitOn}`);
          break;
        }
      }
      ...
      if (result.results.length > 0) {
        ...
        if (lastResult.structured) {
          options.variables = {
            ...options.variables,
            input: lastResult.structured,
          };
        } else {
          options.variables = {
            ...options.variables,
            input: lastResult.content,
          };
        }
      }
    }
  ...
  async executeDirectory(
    ...
    try {
      ...
      log.step(`Executing directory`, dirPath);
      ...
      for (let i = 0; i < prompts.length; i++) {
        ...
        log.step(`  [${i + 1}/${prompts.length}]`, name);
